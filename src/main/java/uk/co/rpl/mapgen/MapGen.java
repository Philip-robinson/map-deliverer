...
public class MapGen {
    ...
    public static void main(String[] argv) throws IOException{
        ...
        if (f.exists()) try(InputStream is = new FileInputStream(f)){
            l4jprops.load(is);
                    ...
                    for (String pair: params){
                        try{
                            ...
                            if (nv.length==2){
                                switch(nv[0]){
                                    case "north":
                                        north=Double.parseDouble(nv[1]);break;
                                    case "east":
                                        east=Double.parseDouble(nv[1]);break;
                                    case "scale":
                                        scale=Double.parseDouble(nv[1]);break;
                                    case "width":
                                        width=Integer.parseInt(nv[1]);break;
                                    case "height":
                                        height=Integer.parseInt(nv[1]);break;
                                    default:LOG.warn("Unknown parameter "+pair);
                                }
                            }else addErr.accept("No Parameter value for "+pair);
                        }catch(NumberFormatException e1){
                            addErr.accept("Failed to process "+pair);
                        }
                    }
                    if(error.length()==0){
                        if (north==null) addErr.accept("north must be provided");
                        if (east==null) addErr.accept("east must be provided");
                        if (scale==null) addErr.accept("scale must be provided");
                        if (width==null) addErr.accept("width must be provided");
                        if (height==null) addErr.accept("height must be provided");
                    }
                    if (error.length()==0){
                        XY size = new XY(width, height);
                        XYD eastnorth = new XYD(east, north);
                        XYD scaling = new XYD(scale, -scale);
                        XYD enTC = eastnorth.sub(scaling.mul(
                                              size.xyd()).divide(2));
